...
export type DomeGalleryImage = string | ImageObject;
...
interface DomeItem extends DomeCoords {
  src: string;
  alt: string;
}
...
function buildItems(pool: DomeGalleryImage[], seg: number): DomeItem[] {
  const xCols = Array.from({ length: seg }, (_, i) => -37 + i * 2);
  const evenYs = [-4, -2, 0, 2, 4];
  const oddYs = [-3, -1, 1, 3, 5];

  const coords: DomeCoords[] = xCols.flatMap((x, c) => {
  ...
  const normalizedImages: { src: string; alt: string }[] = pool.map(image => {
  ...
  const usedImages: { src: string; alt: string }[] = Array.from({ length: totalSlots }, (_, i) => normalizedImages[i % normalizedImages.length]);
    ...
    (el: HTMLElement) => {
      ...
      if (wantsResize) {
        const onFirstEnd = (ev: TransitionEvent) => {
          ...
          overlay.removeEventListener('transitionend', onFirstEnd as unknown as EventListener);
        ...
        overlay.addEventListener('transitionend', onFirstEnd as unknown as EventListener);
      }
    },
